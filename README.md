...
React JS is a javascript UI library trying to solve the issue of dynamically update an interface, meaning reloading elements of (or the whole) page without needing to refresh. Instead of the page being resent by the server at every change, React will render bits of the page as needed. 
For this, React JS will be using a virtual DOM kept in memory, which enable us to interpolate variables in an html-like code. Only changes (differences between the DOM and virtual DOM) to components are rendered, when state or props passed to children force a re-render. Another library (babel) can then transpile it into regular HTMl and JS understood by all browsers.

- [x] What does it mean to _think_ in react?

To think in react describe the process of creating an interface in React based on an UI Layout, and based on the React principles. The UI is broken down in components and sub-components, which then enable to pass the data as needed with state and props to children.

- [x] Describe state.
State is an object which represents what can change on a page. When a call for a state changes is made by any component, a re-render happens and difference to the state will dynamically show on the page.

- [x] Describe props.

Props are properties of the state object passed down (one direction) to child components and can't change (immutable). This makes it possible to have components share information between them and trigger changes in the state.
